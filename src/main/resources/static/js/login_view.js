...
/*function loginCode(){
    $("#loginCode").attr("src","/defaultKaptcha?date="+new Date()*1);
}*/


$(".loginBtn").click(function () {
    var userName = $(".login_input [name='userName']").val().trim();
    var userPass = $(".login_input [name='userPass']").val().trim();
    var rememberMe = $(".login_input input[name='rememberme']").is(':checked');
    var tryCode = $(".login_input [name='tryCode']").val().trim();
    console.log(tryCode)
    $.ajax({
        url:href+"login",
        type:"POST",
